...
impl Store {
    ...
    pub fn activate_generation(&self, id: u64) -> Result<()> {
        ...
        let ld_library_path_value = lib_dirs.join(":");
        ...
        if !ld_library_path_value.is_empty() {
            session_env_content.push_str(&format!("\nLD_LIBRARY_PATH={}\n", ld_library_path_value));
        }
        fs::write(&session_env_path, &session_env_content)?;

        if !path_value.is_empty() {
            variables_sh_content.push_str(&format!("export PATH={}\n", path_value));
        }
        if !ld_library_path_value.is_empty() {
        ...
        let systemd_conf_dir = Path::new("/etc/systemd/system.conf.d");
        fs::create_dir_all(systemd_conf_dir)?;
        let systemd_conf_path = systemd_conf_dir.join("rscm.conf");
        let mut systemd_content = String::from("# This file was generated by rscm\n[Manager]\n");
        if !path_value.is_empty() {
            systemd_content.push_str(&format!("DefaultEnvironment=\"PATH={}\"", path_value));
            if !ld_library_path_value.is_empty() {
                systemd_content
                    .push_str(&format!(" \"LD_LIBRARY_PATH={}\"", ld_library_path_value));
            }
            systemd_content.push('\n');
        }
        fs::write(systemd_conf_path, systemd_content)?;

        println!("Switched to generation {}", id);
        if !path_value.is_empty() {
            println!("PATH set to: {}", path_value);
        }
        if !ld_library_path_value.is_empty() {
            println!("LD_LIBRARY_PATH set to: {}", ld_library_path_value);
        }
